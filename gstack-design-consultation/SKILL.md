...
description: |
  ...
  NOT for Google Stitch output — use stitch-skill (stitch-design-taste) for that;
  both skills write a file named DESIGN.md and will overwrite each other.
...
**Step 0: Fetch relevant DESIGN.md references from awesome-design-md**

Before doing web research, pull 2-3 brand design systems from the community library
that match the project type. This gives grounded, proven references rather than
invented-from-scratch tokens.

Lookup map — project type → exact folder names to fetch:
| Project type | Fetch these (exact folder names) |
|---|---|
| Consumer app / food & beverage / franchise | airbnb, spotify, uber |
| Business dashboard / scheduling / ops tool | linear.app, notion, airtable |
| Financial / payment / data-heavy | stripe, wise, coinbase |
| AI product / tech platform | claude, cohere, elevenlabs |
| Marketing / landing page | webflow, framer, vercel |
| Enterprise / B2B SaaS | zapier, supabase, mongodb |

Full list of available folders: airbnb, airtable, apple, bmw, cal, claude, clay, clickhouse, cohere, coinbase, composio, cursor, elevenlabs, expo, figma, framer, hashicorp, ibm, intercom, kraken, linear.app, lovable, minimax, mintlify, miro, mistral.ai, mongodb, notion, nvidia, ollama, opencode.ai, pinterest, posthog, raycast, replicate, resend, revolut, runwayml, sanity, sentry, spacex, spotify, stripe, supabase, superhuman, together.ai, uber, vercel, voltagent, warp, webflow, wise, x.ai, zapier

Fetch each using WebFetch on (use exact lowercase folder name):
`https://raw.githubusercontent.com/VoltAgent/awesome-design-md/main/design-md/[exact-folder]/DESIGN.md`
...
Or via bash:
```bash
curl -s "https://raw.githubusercontent.com/VoltAgent/awesome-design-md/main/design-md/[exact-folder]/DESIGN.md" 2>/dev/null | head -80
```

After fetching, extract from each: color philosophy, type approach, spacing system,
component personality. Synthesize patterns — do NOT copy. The goal is informed
inspiration, not imitation. State clearly which patterns you are borrowing and why.

If fetch fails (network issue), skip gracefully and continue with built-in knowledge.
...
| [today] | Referenced [Brand1], [Brand2] from awesome-design-md | [what was borrowed — e.g., spacing system from Linear, color philosophy from Airbnb] |
```
